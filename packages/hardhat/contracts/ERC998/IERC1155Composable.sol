...
interface IERC1155Composable {
    ...
    event TransferBatchChildTokens(
        uint256 indexed _fromTokenId, 
        address indexed _recipientTokenId, 
        address indexed _childContract, 
        uint256[] _childTokenIds, 
        uint256[] _amounts
    );
    ...
    // function childContractsFor(uint256 _tokenId) external view returns (address[] memory childContracts);
    function childTokensFor(uint256 _tokenId, address _childContract) external view returns (uint256[] memory childTokens);
