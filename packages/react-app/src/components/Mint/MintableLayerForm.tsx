...
const MintableLayerForm: React.FC<MintableLayerFormProps> = ({ layer }: MintableLayerFormProps) => {
  ...
  const onFormValuesChange = (changedValues, allValues) => {
    ...
    if (changedValues.recipient && changedValues.recipient.address)
      layer.setRecipientAddress(changedValues.recipient.address);
  ...
  return (
    <Card>
      <Form name={layer.id} labelCol={{ span: 4 }} wrapperCol={{ span: 18 }} onValuesChange={onFormValuesChange}>
        <Row gutter={4}>
          <Col span={16}>
            ...
            <Form.Item name={['description']} label="Description">
              <TextArea rows={4} autoSize placeholder="What is this layer all about?" />
            </Form.Item>
            <Form.Item
              // name="recipient"
              label="Recipient"
