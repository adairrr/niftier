...
export class EventModelSelector extends QueryBuilder {
  ...
  transaction(
    builder?: string | TransactionModelSelector | ((selector: TransactionModelSelector) => TransactionModelSelector),
  ) {
    return this.__child(`transaction`, TransactionModelSelector, builder);
  }
  approval(builder?: string | ApprovalModelSelector | ((selector: ApprovalModelSelector) => ApprovalModelSelector)) {
    return this.__inlineFragment(`Approval`, ApprovalModelSelector, builder);
  }
  transfer(builder?: string | TransferModelSelector | ((selector: TransferModelSelector) => TransferModelSelector)) {
    return this.__inlineFragment(`Transfer`, TransferModelSelector, builder);
  }
}
